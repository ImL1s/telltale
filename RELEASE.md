...
## 4.5 送審之前：Play 上那份草稿裡的 AAB 現在是**不可散布**的

**這一條比整份文件的其他部分都急，因為它會被一個按鈕悄悄推翻。**

目前掛在正式版草稿上、也正在內部測試軌道服務的那個 bundle 是 **versionCode 1，
2026-08-17 建的** —— 那是 BLE 套件置換**之前**。它裡面是 `flutter_blue_plus`，
而該套件的授權 Section 3 明文禁止付費散布（見 `SPEC_DEVIATIONS.md` §5）。

換句話說：**原始碼已經修好了，Play 上那個產物沒有。** 如果有人在正式版草稿上直接
按下送審，送出去的就是那個舊 bundle，US$4.99 上線，正好是整個置換要避免的那件事 ——
被一個按鈕原地復活。

所以送審前的順序是硬性的：

1. 確認本機 `git log` 已含 BLE 置換（`grep -n universal_ble pubspec.yaml` 必須有命中，
   而 `flutter_blue_plus` 只該出現在解釋為什麼不用它的註解裡）
2. 依第 1 節把 `version:` bump 到 `+2` 以上
3. 重新 `flutter build appbundle --release`
4. **上傳這個新 AAB 取代草稿上的那個**，再走第 5 節的實機閘門
5. 才輪到送審

只有第 4 步完成之後，「M17 授權問題已關閉」這句話對**散布**才成立；在那之前它只對
**原始碼**成立。這個區別在 2026-08-18 那天差點被漏掉。
