...
void main() {
  ...
  test('a handshake that fails still leaves its bytes behind', () async {
    ...
    expect(session.client, isNull, reason: 'the session is gone, as it should be');
    ...
    expect(text, contains('# 連線方式：Wi-Fi'),
        reason: 'a record with no idea what produced it is most of the way to '
            'useless, and the header is rendered while the client still exists');
  });

  test('a second attempt does not erase the first attempt evidence early',
      () async {
  ...
  test('a transport that never connected still leaves a record', () async {
    ...
    expect(text, contains('連線失敗'),
        reason: 'and what it failed with, which is the whole point');
  });

  test('R28-N6: the heading always describes the transcript it sits on',
      () async {
    ...
    expect(record.header, contains(device),
        reason: 'the heading describes the bytes underneath it or it is worse '
  ...
  test('a live session exports the live transcript, not a stale one', () async {
    ...
    expect(session.exportableTranscript, isNot(same(failed)),
